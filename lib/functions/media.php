...
function radium_resize( $url, $width, $height = null, $crop = null, $single = true ) {
...
function radium_cleaner_caption( $output, $attr, $content ) {
...
add_filter( 'img_caption_shortcode', 'radium_cleaner_caption', 10, 3 );
...
if ( !function_exists( 'radium_gallery' ) ) {
    function radium_gallery($postid, $image_w, $image_h = null, $crop = null ) { 
	...
	$bg_images = get_post_meta( $postid, '_bean_bgimage', false );
			          ...
			          echo '
	          		    <ul class="slides">';
					          ...
					          foreach( $attachments as $attachment ) {
						          ...
						          $image = radium_resize( $src[0], $image_w, $image_h, $crop );
...
if ( !function_exists( 'radium_audio' ) ) {
	...
	function radium_audio($postid) {

		$mp3 = get_post_meta($postid, '_bean_audio_mp3', TRUE);
		$ogg = get_post_meta($postid, '_bean_audio_ogg', TRUE);
		$poster = get_post_meta($postid, '_bean_audio_poster', TRUE);
		$height = get_post_meta($postid, '_bean_poster_height', TRUE);
		//$width = get_post_meta($postid, '_bean_poster_width', true);
...
/* OUTPUT VIDEO
/*-----------------------------------------------------------------------------------*/
function radium_video($postid) {
	
	$height = get_post_meta($postid, '_bean_video_height', true);
	$m4v = get_post_meta($postid, '_bean_video_m4v', true);
	$ogv = get_post_meta($postid, '_bean_video_ogv', true);
...
/*-----------------------------------------------------------------------------------*/
function radium_hex_addition( $hex, $num ){
	$rgb = radium_rgb2hex( $hex );
	foreach ( $rgb as $key => $val ) {
		$rgb[$key] += $num;
		$rgb[$key] = ( $rgb[$key] < 0) ? 0 : $rgb[$key];
	}
	$hex = radium_rgb2hex( implode( ',', $rgb ) );
